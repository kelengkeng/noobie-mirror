...
def select(update, context):
    ...
    if len(context.args) == 1:
        ...
        if not dl:
            sendMessage(f"{tag} GID: <code>{gid}</code> Not Found.", context.bot, update.message)
            return
        ...
        with download_dict_lock:
            if mirror_message.message_id in download_dict:
                ...
                dl = None
        if not dl:
            sendMessage(f"{tag} This is not an active task!", context.bot, update.message)
            return
    elif len(context.args) == 0:
        g = "Reply to an active /cmd which was used to start the qb-download or add gid along with cmd\n\n"
    ...
    if OWNER_ID != user_id and dl.message.from_user.id != user_id and \
       (user_id not in user_data or not user_data[user_id].get('is_sudo')):
        tmsg = sendMessage(f"{tag} This task is not for you!", context.bot, update.message)
    ...
    if dl.status() not in [MirrorStatus.STATUS_DOWNLOADING, MirrorStatus.STATUS_PAUSED, MirrorStatus.STATUS_WAITING]:
        tmsg = sendMessage(f"{tag} Task should be in download or pause (incase message deleted by wrong) or queued (status incase you used torrent file)!", context.bot, update.message)
        Thread(target=auto_delete_message, args=(context.bot, update.message, tmsg)).start()
        return
    if dl.name().startswith('[METADATA]'):
        tmsg = sendMessage(f"{tag} Try after downloading metadata finished!", context.bot, update.message)
        Thread(target=auto_delete_message, args=(context.bot, update.message, tmsg)).start()
        return
    try:
        ...
        tmsg = sendMessage(f"{tag} This is not a bittorrent task!", context.bot, update.message)
        Thread(target=auto_delete_message, args=(context.bot, update.message, tmsg)).start()
        return

    SBUTTONS = bt_selection_buttons(id_)
    msg = f"{tag} Your download paused. Choose files then press Done Selecting button to resume downloading."
